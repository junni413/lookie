...
const devBypassPhone = true;   // ✅ 전화번호 중복확인만 무시
const devBypassEmail = false;  // ✅ 이메일은 실제 발송/검증
...
export default function Signup() {
  ...
  // ✅ 전화번호 중복확인 (devBypassPhone=true면 API 호출 안 함)
  const handlePhoneDup = async () => {
    const digits = onlyDigits(phone);
    ...
    if (devBypassPhone) {
      setPhoneVerified(true);
      setErrors((prev) => ({ ...prev, phone: undefined }));
      alert("DEV: 전화번호 중복확인 생략(확인완료 처리)");
      return;
    }
  ...
  // ✅ 이메일 인증번호 발송 (devBypassEmail=false면 실발송)
  const handleEmailSendCode = async () => {
    if (!isValidEmail(email)) {
      setErrors((prev) => ({ ...prev, email: "이메일 형식을 확인해주세요." }));
      return;
    ...
    if (devBypassEmail) {
      ...
      setCooldownUntil(Date.now() + 60 * 1000);
      setErrors((p) => ({ ...p, email: undefined, emailCode: undefined }));
      alert("DEV: 이메일 발송 생략(코드 입력 단계로 이동)");
      return;
    }
  ...
  // ✅ 이메일 인증번호 검증 (devBypassEmail=false면 실검증)
  const handleEmailConfirmCode = async () => {
    if (!isValidEmail(email)) return;
    if (!emailCode.trim()) {
      setErrors((p) => ({ ...p, emailCode: "인증번호를 입력해주세요." }));
